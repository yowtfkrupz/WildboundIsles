# Wildbound Isles

**Wildbound Isles** is a **single-player survival game** inspired by **Muck**. Players are stranded on procedurally generated islands, facing various challenges like crafting, surviving, and fighting enemies.

## Features
- **Procedural World Generation**: Unique islands with resources and biomes like beaches or plains.
- **Crafting & Survival**: Collect resources, craft items, and stay alive.
- **Enemies & Boss Fights**: Encounter various enemies and face off against powerful bosses, including The Lich.
- **Interactive Obelisks**: Activate obelisks to summon challenging bosses.

## Installation
1. Clone the repository:
    ```bash
    git clone https://github.com/yowtfkrupz/WildboundIsles.git
    ```
2. Open the project in **Unity**.
3. Ensure compatibility with your Unity version.

## License
This project is licensed under the **MIT License**.
